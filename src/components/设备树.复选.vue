...
const handleTreeCheck = (_data: unknown, checkedInfo: { checkedNodes?: unknown[] }) => {
  const nodes = checkedInfo?.checkedNodes ?? [];
  const ids = new Set<string>();
  for (const n of nodes as { isGroup?: boolean; children?: unknown[]; id?: string }[]) {
    if (n?.isGroup) {
      for (const c of n?.children ?? []) {
        const cNode = c as { id?: string; isGroup?: boolean };
        if (cNode?.id && !cNode?.isGroup) ids.add(String(cNode.id));
      }
    } else if (n?.id) {
      ids.add(String(n.id));
    }
...
<template>
  <div class="security-policy-device-tree">
